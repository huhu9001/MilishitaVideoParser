...
static void report_decode_progress() {
	...
	while (std::this_thread::sleep_for(one_second), n_frame != -1)
		std::cout << "frame: " << n_frame << " of " << n_frame_all << std::endl;
}
		...
		if (fs.error()) {
			...
			for (noteParser::note const n_this : np.notesOutput) {
				if (!(n_this.time - time_pre <= noteParser::time_error_permitted && time_pre - n_this.time <= noteParser::time_error_permitted)) {
					f_o << std::endl;
					f_o << "Dialogue: 0,";
					if (int64_t const hour = n_this.time / 3600000; hour > 0) {
						f_o << hour;
						f_o << ':';
					}
					f_o << std::setw(2) << n_this.time / 60000 % 60;
					f_o << ':';
					f_o << std::setw(2) << n_this.time / 1000 % 60;
					f_o << '.';
